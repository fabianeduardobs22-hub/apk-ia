...
def dependency_status() -> None:
    ...
    for cmd in deps:
        ...
        if path:
            print(color(f"  ✔ {cmd:<16} en {path}", "92"))
            continue

        if current_platform == "termux" and cmd in {"bluetoothctl", "hcitool", "sdptool", "btmgmt", "bluetoothd"}:
            print(color(f"  ⚠ {cmd:<16} no disponible en esta instalación Termux (limitación de Android/paquetes)", "93"))
        else:
            print(color(f"  ✖ {cmd:<16} no instalado", "91"))
